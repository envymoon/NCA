"""Clean Branch L: paired transfer test for geometry-aware NCA step budgeting.

This is the submission-grade rerun entry point.  It intentionally does not reuse the
analysis or conclusions in branch_L_transfer.py.  It only reuses its independently checked
weighted shortest-path pool builder.

Design locked here:
  * G=32, het in {0.0, 0.9}, three model seeds.
  * One fixed anytime cap (152), matching the final Branch E run.
  * The same maze and the same underlying uniform medium sample at every het level.
  * A three-channel model at both levels.  When het=0, its active subnetwork is initialized
    exactly as a two-channel BF.NCA, so the dormant medium channel cannot change the control.
  * Mean and p95-per-cell-error knees are both retained for every validation scene.
  * Clamped, trivial, and non-convergent scenes are recorded rather than silently dropped.
  * No Boolean "circularity broken" claim is manufactured by the training script.

The default command refuses to overwrite an existing result.  Use --preflight_only for a
small CPU-safe setup check that performs no training and writes no result.
"""
...
def write_json(path, obj):
    """Strict JSON only: NaN/Infinity are errors, never silent output."""
    path = Path(path)
...
def compatible_nca3(seed, device):
    """Three-input NCA whose het=0 active subnetwork matches a two-input BF.NCA at init.

    The medium plane is the final input channel and is identically zero at het=0.  Copying the
    two-input weights removes the fan-in initialization change caused by merely adding that
    dormant channel.  The medium-specific weights remain trainable for het>0.
    """
    BF.set_seed(seed)
    ...
    # Make batch/horizon sampling identical between paired het levels.
...
def assert_control_compatibility(device):
    ...
    # Copy model2 into a fresh extended model without depending on RNG equality.
...
def make_paired_pools(args, device, small=False):
    ...
    for name, collection in (("train", pools), ("validation", vpools)):
        for het in args.hets[1:]:
            # seed, obstacle, Dgeo, Dfree, Deuc and tau must be exactly paired.
    ...
    # het=0 must reduce exactly to the normalized unweighted hop-distance target.
    if 0.0 in pools:
