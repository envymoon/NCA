"""Real-geometry blind test: frozen maze-trained NCA on VGLC game levels (eval only).

POST-FREEZE ADDENDUM (2026-07-17). Nothing is retrained, no frozen artifact is touched,
no policy constant is refitted on the new data. Writes ONLY eval_vglc.json / .log.

DATA (external, ../data/TheVGLC, github.com/TheVGLC/TheVGLC, sparse checkout):
  Lode Runner  150 levels, 32x22 chars -> centered in 32x32 with obstacle padding.
               obstacle = {B, b} (solid); passable = {., -, #, G, M, E} (E = enemy:
               damaging but not solid geometry).                    -> G=32 checkpoints
  Zelda        18 dungeon maps partitioned into 11x16-tile rooms; non-void rooms
               deduped by layout -> padded to 16x16 with obstacle cols.
               obstacle = {W, B, P, O, I, -}; passable = {F, D, S, M}. -> G=16 checkpoints

PROTOCOL (byte-for-byte the frozen pipeline's):
  target   = BF.bfs8 distance, unreachable/clamped at Dnorm = 2*max(H,W), /Dnorm
  mask     = reach * free  (reachable free cells only)
  var      = tgt[mask>0].var() per (game) pool                (same rule as branch E)
  curve    = BF.tail_nmse(q=0.95) at every t in 1..CAP=152    (frozen global cap)
  knee     = BF.abs_knee(theta=0.05, persist=10, win=5); None = censored
  rulers   = Dgeo / Dfree / Deuc to the SAME geodesically-farthest reachable cell
  eligible = unclamped scenes (Dgeo <= 2G), identical to the audit's rule
  seeds    = per level, K placements drawn from free cells (fixed RNG), re-drawn up to
             8 tries until reach >= 0.20 * free  (make_maze's min_reach rule); else skip
  models   = frozen E checkpoints s3 AND s4 per grid (no selection between them)

QUESTIONS FIXED BEFORE THE FIRST RUN, answered in this order:
  (a) COVERAGE   what fraction of eligible real-geometry scenes converge at all
                 (knee not None)?  Context number: maze ceiling was 94.2%.
  (b) ORDERING   on converged rows, is CV(T*/Dgeo) < CV(T*/Dfree) and < CV(T*/Deuc)?
                 Gate: geo strictly smallest in every (game x checkpoint) cell.
  (c) CALIBRATION do MAZE-fitted constants (audit one_constant and the
                 per-grid f=0.95 fits, both read from audit_full_population.json rows,
                 never refitted) deliver nominal coverage on real maps, unconditionally
                 (censored = unserved)?  Gate: geo coverage within 5pp of the flat
                 baseline fitted the same way, and the geo-vs-flat cost gap preserved.
  A VGLC-fitted c is also printed as a DIAGNOSTIC ONLY (labelled as such) to show how
  far the maze constant sits from the local one; it enters no gate.

Run (cuda):
  cd experiments && python eval_vglc.py

CPU-only policy correction from the already stored per-scene rows:
...
def load_lode_runner():
    """150 levels 32x22 -> (name, obs 32x32) with 5 obstacle rows top+bottom."""
    out = []
...
def load_zelda_rooms():
    """Partition each dungeon into 11x16 rooms, keep non-void, dedupe by layout,
    pad to 16x16 (2 obstacle cols left, 3 right)."""
    seen, out = set(), []
        ...
        for ry in range(H // 16):
            for rx in range(W // 11):
                ...
                if all(set(t) <= {"-"} for t in tile):
                    continue                                   # void cell of the map
                key = "\n".join(tile)
                if key in seen:
                    continue                                   # duplicate room layout
                seen.add(key)
                obs = []
                for t in tile:
                    row = [1.0, 1.0]
...
def build_scenes(levels, G, k_seeds, rng):
    """Seed placement per level, maze-family acceptance rule (reach >= 0.20 free)."""
    seeds, obses, names = [], [], []
...
def per_scene_knees(model, inp, tgt, mask, var):
    """tail-NMSE curve at every t in 1..CAP, sliced; then abs_knee per scene."""
