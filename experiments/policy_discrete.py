"""Shared integer-step provisioning policy used by all paper analyses.

An NCA executes an integer number of update passes.  The deployed geometric policy is

    T_i(c) = max(1, ceil(c * D_i)).

Constants are stored to two decimal places and calibrated on that same discrete policy.
For a flat target budget, ``fit_c_to_coverage`` returns the smallest cent-valued constant
whose unconditional coverage reaches the requested target.  Censored rows remain in the
denominator and can never be served.
"""
import math
...
def eval_policy(rows, ruler, constant, knee):
    """Return mean integer passes and unconditional coverage."""
    if ruler == "flat":
...
def fit_c_to_coverage(rows, ruler, target_coverage, knee,
    ...
    """Smallest resolution-spaced c whose unconditional coverage reaches target."""
...
def fit_matched_policy(rows, ruler, f, knee, resolution=C_RESOLUTION):
    """Fit a ruler policy to the coverage delivered by the flat f-quantile budget."""
