"""Was c1's flatness bought by silently dropping the hard scenes?

branch E dumps only scenes that converged under BOTH knee definitions. If the dropped
scenes concentrate at high D_geo or high tau, then "c1 flat across grids" could be
censoring, not sufficiency, and the per-grid convergence fraction becomes a claim the
paper must defend. The dropped scenes are not in the JSON -- but the val pool is
deterministic (build_pool_maze with generator seed=1 on the SAME device as the run), so
it can be rebuilt bit-exactly and every dumped row matched back to its pool index. The
unmatched scenes ARE the dropped ones, with their D_geo and tau recovered.

Device matters: torch.Generator streams differ between cpu and cuda, so this must run on
the device the training run used (--device cuda for the real runs).

Run: python3 check_censoring.py results_E_maze_s012.json --device cuda
"""
import argparse
import json
...
def match_rows(blk, Dgeo, Dfree, Deuc):
    """Match dumped rows back to pool indices by the (dgeo, dfree, deuc) triple."""
    key = {}
...
def quartile_report(vals, kept, name):
    """Kept fraction inside each quartile of the FULL pool's distribution of `vals`."""
