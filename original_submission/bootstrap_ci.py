"""Scene-bootstrap confidence intervals for the paper's headline numbers. CPU only.

POST-FREEZE ADDENDUM (2026-07-17). Reads ONLY audit_full_population.json (frozen row
dump); writes ONLY bootstrap_ci.json / .log. Closes the SUBMISSION_READINESS P1 item
"include uncertainty across model seeds and scene bootstrap intervals".

RESAMPLING UNIT: the SCENE (grid, pool_idx), stratified by grid -- a resampled scene
carries all five of its seed-rows, so within-scene correlation across model seeds is
preserved (resampling rows independently would understate the intervals). Censored rows
travel with their scene. B=2000, percentile 2.5/97.5, RNG fixed.

WHAT GETS AN INTERVAL (the numbers the paper actually quotes; the fitting procedure is
re-run INSIDE each replicate, so the interval covers fit noise too):
  1. fitted-f savings, f in {0.90, 0.95, 0.99}: 1 - mean_cost(geo)/T_flat with the
     integer-pass policy refitted on each replicate
  2. one-constant fixed at the corrected audit's c/T, per-grid unconditional coverage
  3. per-grid CV gap on converged rows: CV(T*/Dfree) - CV(T*/Dgeo) and
     CV(T*/Deuc) - CV(T*/Dgeo); ordering claim is supported if the 2.5th percentile
     of the gap stays > 0                          (gate: all six gaps positive at 2.5%)
  4. convergence ceiling                           (paper: 94.2%)

Run:  cd experiments && python bootstrap_ci.py
"""
import json
import random
...
def stats_of(pop_by_grid, c_fix, t_fix):
    """All headline numbers from one (re)sampled population."""
...
def main():
    ...
    scenes = {}                                    # (grid, pool_idx) -> 5 seed-rows
    for r in rows:
