...
fig, axes = plt.subplots(1, 3, figsize=(7.5, 2.35),
                         gridspec_kw={"width_ratios": [1, 1.22, 1.05], "wspace": 0.55})
...
cb.ax.set_title(r"$\tau$", fontsize=8)
...
for off, ruler, color, name in ((-w, "dgeo", C_GEO, "geodesic"),
...
axes[2].set_title("(c) ruler dispersion (tail knee)")
axes[2].legend(frameon=False)
fig.savefig("figs/fig1_ruler.pdf"); fig.savefig("figs/fig1_ruler.png")
plt.close(fig)

# ---------------------------------------------------------------- figure 2
fig, axes = plt.subplots(1, 2, figsize=(7.1, 2.5), gridspec_kw={"wspace": 0.28})
...
ax.errorbar(grids, c1m, yerr=c1s, color=C_GEO, marker="o", ms=4, capsize=2,
            label=r"per-scene tail knee $/\,D_\mathrm{geo}$")
E = json.load(open(aud["source"]))
...
ax.errorbar(grids, [agg[str(G)]["mean"] for G in grids],
            ...
            label=r"aggregate mean knee $/\,\overline{D_\mathrm{geo}}$")
...
ax.text(grids[-1] - 0.3, ref[-1] + 0.015, r"$D\log D$ law (ref.)",
        ha="right", va="bottom", fontsize=6.5, color="0.15")
ax.axhline(1.0, color="0.75", lw=0.7)
ax.set_xticks(grids); ax.set_xlabel("grid size $G$")
ax.set_ylim(top=max(ref[-1], max(c1m)) + 0.12)
ax.set_ylabel(r"$c_1 = \overline{T^*/D_\mathrm{geo}}$")
ax.set_title("(a) near-linear scaling of the budget")
...
for ruler, color, name in (("flat", C_FLAT, "flat (worst-case) $T$"),
                           ("dgeo", C_GEO, r"$T=c\,D_\mathrm{geo}$"),
                           ("dfree", C_FREE, r"$T=c\,D_\mathrm{free}$"),
                           ("deuc", C_EUC, r"$T=c\,D_\mathrm{euc}$")):
    ...
    ax.plot([p[0] for p in pts], [100 * p[1] for p in pts], color=color,
            ...
            ls="-" if ruler in ("dgeo", "flat") else "--")
ax.grid(axis="y")
ax.set_axisbelow(True)
ax.axhline(100 * ceiling, color="0.6", lw=0.7, ls=":")
ax.text(97, 100 * ceiling + 0.4, f"convergence ceiling {100*ceiling:.1f}%",
...
ax.set_ylabel("scenes served, unconditional (%)")
ax.set_title("(b) cost vs. coverage, all grids pooled")
ax.legend(frameon=False, loc="lower right")
fig.savefig("figs/fig2_policy.pdf"); fig.savefig("figs/fig2_policy.png")
plt.close(fig)
