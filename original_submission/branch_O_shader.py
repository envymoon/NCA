"""
Branch O — deployment-cost measurement: the NCA step as a real fused GPU kernel.

WHAT THIS IS AND IS NOT
  This is a SYSTEMS MEASUREMENT, not a hypothesis test: no training, no gates, no fitting.
  Every policy constant is read from the frozen audit (audit_full_population.json,
  policy_full): at service target f the flat arm provisions T_flat passes for every
  scene and the geodesic arm provisions T_i = max(1, ceil(c_geo * max(Dgeo_i, 1)))
  with the SAME pooled constant the paper reports (f=.95: T_flat=39, c_geo=1.29).
  Nothing is refit here; if a constant were refit for wall-clock, the timing table
  would no longer describe the paper's policy.

WHY A FUSED KERNEL AND NOT THE PyTORCH LOOP
  At G=16..32 a single NCA step is ~5.5k MAC/pixel on <=1024 pixels: the eager loop
  is dominated by Python dispatch and kernel-launch latency, so re-timing it would
  measure the harness, not the shader. The deployment analogue is built instead:
    k0  frame reset: state=0, static planes [seed, obstacle] read in place
    k1  perception: depthwise 3x3, 14 ch -> 42 ch (zero padding), fp32
    k2  update: 1x1 (42->96, zero-padded to 64 lanes) + ReLU + 1x1 (96->12) + residual, fp32 IEEE tl.dot
    k3  readout: 1x1 (12->1)
  in channel-last layout (HW x 16: cols 0-11 state, 12-13 static input, 14-15 zero
  lanes), and ONE FRAME = k0 + T x (k1,k2) + k3 is captured as a single CUDA graph
  per budget T and replayed -- the command-buffer execution model of a shader
  runtime. The graph contains 2T+2 kernel nodes and is submitted by one graph replay;
  synchronization is performed by the timing harness after replay. BOTH ARMS RUN THE
  IDENTICAL KERNELS AND GRAPH MACHINERY; they differ only in T.

QUALITY MATCH (why the timing table inherits the paper's coverage numbers)
  Parity phase: for every val scene (deterministic pool, generator seed 1, the same
  pool every quality number in the paper is computed on) the kernel path's per-scene
  tail-NMSE curve over t=1..cap is compared to the PyTorch module's, from the same
  archived checkpoint; per-scene tail knees (BF.abs_knee, theta=0.05) must agree.
  On each checked checkpoint/grid unit, matching knees imply matching per-scene service
  (knee_i <= budget_i). Six archived units are checked; the timing path itself is
  weight-independent. Policy coverage remains the frozen audit's result rather than a
  new coverage estimate. Max |readout| deviation is reported alongside.

ONE-TIME COST AND THE TWO GEOMETRY REGIMES
  The geodesic arm must pay for one BFS on the obstacle mask. It is timed as a
  NATIVE cost: a 30-line C BFS (8-connected, array queue, -O2) compiled at run time
  and called per scene; its Dgeo must equal the pool's archived Dgeo for every scene.
    STATIC geometry: BFS runs once, then amortises; break-even frame count
      n* = t_BFS / (t_flat - t_geo_mean) is reported per grid.
    PER-FRAME geometry: BFS is paid every frame; net per-frame saving
      (t_flat - t_geo_mean) - t_BFS is reported with its sign.

TIMING PROTOCOL (v2, replacing the single-block pilot)
  Batch = 1 scene ("one screen per frame"). Per unique budget T: 25 warm-up replays,
  then 25 independent blocked groups of 50 replays. Each group records both CUDA-event
  device time and host perf_counter time; the median group mean is the low-jitter device
  diagnostic. In addition, 100 individual graph replay + synchronize samples record
  host-observed per-frame latency, which is the paper's primary wall-clock quantity.
  Budget order is deterministically shuffled per grid to distribute thermal/order drift.
  Raw samples, medians, p10/p90 intervals, and environment metadata are saved. Per-step
  cost and per-frame overhead are OLS fits against T over all unique budgets. Eager
  PyTorch model.run at T_flat is reported only as a framework contrast, never as an arm.
  Arm cost per grid = mean over ELIGIBLE scenes (clamped Dgeo>2G excluded, exactly
  the audit's population rule) of the frame time at that scene's budget.
  Static input tensors are already GPU-resident. Graph capture/setup, graph-executable
  metadata, changed-mask transfer, policy selection, and renderer integration are outside
  the timed frame; any "dynamic" BFS subtraction is therefore labelled compute-only.

Checkpoint: results_E_maze_s34.json.G{16,24,32}.s3.pt (first archived seed; timing is
weight-independent, parity is run per checkpoint). Memory footprint reported = X + P +
readout + weight buffers. fp32 throughout (matches training); fp16 is future work.

Run: python3 branch_O_shader.py            # all grids, parity + timing + BFS
     python3 branch_O_shader.py --smoke    # G16 kernel-vs-torch smoke test only
"""
...
def k1_perceive(X, P, WP, H: tl.constexpr, W: tl.constexpr,
    ...
    m = idx < NPIX
    hw = idx % (H * W)
    b0 = idx - hw            # batch offset in pixels
    y = hw // W
    x = hw % W
...
def pack_weights(sd, dev):
    wp = sd["perceive.weight"].reshape(14, 3, 3, 3).contiguous()          # (c,f,ky,kx)
...
class Shader:
    """Kernel-path NCA: same weights, channel-last buffers, batch folded into pixels."""
    ...
    def set_scenes(self, inp):
        """inp: (B,2,H,W) torch tensor -> channel-last static planes."""
...
def capture_frame(shader, T):
    shader.frame_ops(T)                                # warm compile outside capture
    torch.cuda.synchronize()
...
def parity_phase(model, shader, vinp, vtgt, vlm, vvar, cap, dev):
    """Kernel vs PyTorch on the full val pool: readout deviation + tail-knee match."""
    B = vinp.shape[0]
    shader.set_scenes(vinp)
    Tlist = list(range(1, cap + 1))
    with torch.no_grad():
        # torch reference curves, batched like the archive run
    ...
    # kernel curves: step the whole pool at once, readout each step
    ps_k = {}
    maxdev = 0.0
    shader.frame_ops(0)                                # k0 only: reset
    for t in Tlist:
...
def main():
    ...
    for G in a.grids:
        ...
        # ---- native BFS one-time cost + Dgeo cross-check ----
        ...
        # ---- graph timing per unique T ----
        # Retain the captured graphs so that timing order can be randomized without
        # mixing graph construction into the measurement.
        ...
        # eager contrast at the primary flat budget
